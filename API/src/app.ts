...
const UBER_ID: string = process.env.UBER_CLIENT_ID ?? "";
const UBER_SECRET: string = process.env.UBER_CLIENT_SECRET ?? "";
...
app.post("/register", async (req, res) => {
  try {
    ...
    const refreshToken = jwt.sign({ _id: body.email }, REFRESH_JWT_SECRET, {
      expiresIn: "7d",
    });
...
app.post("/login", async (req, res) => {
  try {
    ...
    if (!result) {
      res
        .status(400)
        .send({ message: "Login failed: Invalid username/password" });
      return;
    }
...
app.post("/uber-auth", async (req, res) => {
  // fetch("https://login.uber.com/oauth/v2/token");
});

app.get("/uber-auth", async (req, res) => {
